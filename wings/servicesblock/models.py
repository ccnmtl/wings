...
class ServicesBlock(models.Model):
    ...
    def unlocked(self, user):
        if self.page_type == 'page_1':
            return (
                NarrowedDownAnswer.objects.filter(
                    user=user)[0].answer is not None
            )
...
class NarrowedDownAnswer (models.Model):
    """This is to record which of a multiple-choice, multiple-answer
    question (i.e. checkboxes) a user chose as the most important
    one."""
...
class ServiceProvider(models.Model):
    issue = models.ForeignKey(
        Answer,
        null=True,
        limit_choices_to={'question__id': 241})
    ...
    description = models.TextField(
        blank=True,
        null=True,
        help_text="One-sentence description of the service")
