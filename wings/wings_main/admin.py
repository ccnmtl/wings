        ...
        if (participant.has_started_intervention()
            ...
            relaunch_js_href = (
                "javascript:set_no_admin_and_go (\'%s\',  \'%s\')" % (
                    ...
                    launch_url))
            go_button = (
                '<a href="%s" class="relaunch_button" >Relaunch</a>'
                % relaunch_js_href)
            exit_materials_url = "/participant/%s/exit_materials/" % ids
            exit_materials_button = (
                '<a href="%s" class="exit_materials_button" >'
                'Exit materials</a>' % exit_materials_url)
        else:
            # START INTERVENTION:
            ...
            launch_js_href = (
                "javascript:set_no_admin_and_go (\'%s\',  \'%s\')" % (
                    ...
                    launch_url))
            go_button = (
                '<a href="%s" class="start_intervention_button" '
                'id="launch_%s"  >Start intervention</a>' % (
                    launch_js_href,
                    ids))
            exit_materials_button = ''
